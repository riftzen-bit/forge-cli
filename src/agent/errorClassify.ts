export type ClassifiedError = {
  category: 'auth' | 'quota' | 'rate-limit' | 'network' | 'server' | 'not-found' | 'sdk-subprocess' | 'unknown';
  retryable: boolean;
  hint?: string;
  message: string;
  // True when the underlying SDK subprocess died — caller should drop the
  // resumed session id before retrying so the next call starts a fresh
  // child process instead of trying to reattach to the dead one.
  needsFreshSession?: boolean;
};

const PATTERNS: Array<{ re: RegExp; category: ClassifiedError['category']; retryable: boolean; hint?: string; needsFreshSession?: boolean }> = [
  ...
  // Claude Code SDK spawns a subprocess; if it dies (cancellation, OOM,
  // resume failure, etc.) the SDK surfaces "Claude Code process exited
  // with code N". Treat as retryable but drop the resume id first.
  { re: /Claude Code process exited|claude.*exited with code|process exited with code/i, category: 'sdk-subprocess', retryable: true, hint: 'sdk subprocess crashed; restarting fresh session', needsFreshSession: true },
];
...
export function classifyError(err: unknown): ClassifiedError {
  ...
  for (const p of PATTERNS) {
    if (p.re.test(msg)) {
      ...
      if (p.needsFreshSession) out.needsFreshSession = true;
      return out;
    }
  }
